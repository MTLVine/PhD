...
R1 = G2*u2x + kAb0*b2x*1j - alpha*uW*(u1x + u3x)*1j + alpha*bW*(b1x + b3x)*1j - uW*(u1z - u3z)*1j + bW*(b1z - b3z)*1j - alpha*p2*1j + kWx*rho2/kWz 
R2 = G2*u2z + kAb0*b2z*1j - alpha*uW*(u1z + u3z)*1j + alpha*bW*(b1z + b3z)*1j                                         - (gamma + n)*p2*1j - rho2
R3 = G2*b2x + kAb0*u2x*1j + alpha*bW*(u1x + u3x)*1j - alpha*uW*(b1x + b3x)*1j - bW*(u1z - u3z)*1j + uW*(b1z - b3z)*1j
R4 = G2*b2z + kAb0*u2z*1j + alpha*bW*(u1z + u3z)*1j - alpha*uW*(b1z + b3z)*1j
...
Beta = np.linspace(1E-10,4,nBeta)
...
M1 = M1.subs([(kWx,KWx), (kWz,KWz), (b0x,B0x2), (b0z,B0z2), (gamma,Gamma), (uW,UW), (bW,BW), (rhoW,RhoW)])
M2 = M2.subs([(kWx,KWx), (kWz,KWz), (b0x,B0x2), (b0z,B0z2), (gamma,Gamma), (uW,UW), (bW,BW), (rhoW,RhoW)])
M3 = M3.subs([(kWx,KWx), (kWz,KWz), (b0x,B0x2), (b0z,B0z2), (gamma,Gamma), (uW,UW), (bW,BW), (rhoW,RhoW)])

#Growth rates
growth = np.zeros((nAlpha,nBeta))

for k in range(nAlpha):
    
    MM1 = M1.subs(alpha,Alpha[k])
    MM2 = M2.subs(alpha,Alpha[k])
    MM3 = M3.subs(alpha,Alpha[k])
    
    for l in range(nBeta):
    
        MMM1 = MM1.subs(beta,Beta[l])
        MMM2 = MM2.subs(beta,Beta[l])
        MMM3 = MM3.subs(beta,Beta[l])
        ...
        for i in range(nN):     
            for j in range(nN): 
                if i == j:
                    M[5*i:5*i+5,5*j:5*j+5] = MMM2.subs(n,N[i])
                if j == i + 1:
                    M[5*i:5*i+5,5*j:5*j+5] = MMM3.subs(n,N[i])
                if j == i - 1:
                    M[5*i:5*i+5,5*j:5*j+5] = MMM1.subs(n,N[i])
...
plt.pcolor(Alpha,Beta,np.transpose(growth))

#Growth rate against alpha
#np.savez('growth_1d_b0_1', growth = growth, a = a, theta = theta, zeta = zeta, B0 = B0, beta = beta, gamma = gamma, nN = nN)
